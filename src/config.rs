...
impl Config {
    pub fn from_env() -> Self {
        let xray_path =
            env::var("XRAY_PATH").unwrap_or_else(|_| "/opt/homebrew/bin/xray".to_string());
        let config_file_name =
            env::var("CONFIG_FILE_NAME").unwrap_or_else(|_| "xray.json".to_string());

        let current_dir = env::current_dir().unwrap_or_else(|_| ".".into());
        let default_config_path = current_dir
        ...
        let default_fingerprint =
            env::var("DEFAULT_FINGERPRINT").unwrap_or_else(|_| "safari".to_string());
        let loopback_address =
            env::var("LOOPBACK_ADDRESS").unwrap_or_else(|_| "127.0.0.1".to_string());
        let subscription_user_agent =
            env::var("SUBSCRIPTION_USER_AGENT").unwrap_or_else(|_| "v2rayNG/1.8.5".to_string());
